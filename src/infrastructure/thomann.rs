...
static JSON_LD_PRICE: LazyLock<Regex> = LazyLock::new(|| {
    // JSON-LD: "price":"123.45" or "price": 123.45
    Regex::new(r#""price"\s*:\s*"?(\d+[.,]?\d*)"?"#).unwrap()
});

static META_PRICE: LazyLock<Regex> = LazyLock::new(|| {
    // <meta itemprop="price" content="123.45">
    Regex::new(r#"<meta\s+itemprop=["']price["']\s+content=["'](\d+[.,]?\d*)["']"#).unwrap()
});

static THOMANN_PRICE: LazyLock<Regex> = LazyLock::new(|| {
    // Thomann uses a price display: "€ 123,45" or "1.234,56 €"
    // Look for the primary price block.
    Regex::new(r#"(?:class="[^"]*price[^"]*"[^>]*>|data-price=")(\d[\d.]*(?:,\d+)?)"#).unwrap()
});

static TITLE_TAG: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"<title>(.*?)</title>").unwrap());

static CURRENCY_SYMBOL: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"[€$£]|EUR|USD|GBP|PLN").unwrap());
