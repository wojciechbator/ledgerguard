...
static GROSS_PATTERNS: LazyLock<Vec<Regex>> = LazyLock::new(|| {
    vec![
        // Polish: "Brutto: 123,00 zł" / "Wartość brutto: 123,00"
        Regex::new(r"(?i)(?:warto[śs][ćc]\s+)?brutto\s*[:\-]?[ \t]*(\d{1,3}(?:[. ]\d{3})*,\d{2}|\d{1,6},\d{2}|\d{1,6}\.\d{2})\s*(?:z[łl]|pln|eur|usd|€|\$)?").unwrap(),
        // Polish: "Do zapłaty: 123,00 zł" / "Razem: 123,00"
        Regex::new(r"(?i)(?:do\s+zap[łl]aty|razem|suma)\s*[:\-]?[ \t]*(\d{1,3}(?:[. ]\d{3})*,\d{2}|\d{1,6},\d{2}|\d{1,6}\.\d{2})\s*(?:z[łl]|pln|eur|usd|€|\$)?").unwrap(),
        // German: "Gesamtbetrag: 123,00 EUR" / "Endbetrag: 123,00"
        Regex::new(r"(?i)(?:gesamtbetrag|endbetrag|summe)\s*[:\-]?[ \t]*(\d{1,3}(?:[. ]\d{3})*,\d{2}|\d{1,6},\d{2}|\d{1,6}\.\d{2})\s*(?:eur|€)?").unwrap(),
        // Generic: "Total: 123.45"
        Regex::new(r"(?i)total\s*[:\-]?\s*(\d{1,3}(?:,\d{3})*\.\d{2}|\d{1,6}\.\d{2})").unwrap(),
    ]
});

static NET_PATTERNS: LazyLock<Vec<Regex>> = LazyLock::new(|| {
    vec![
        Regex::new(
            r"(?i)(?:warto[śs][ćc]\s+)?netto\s*[:\-]?\s*([\d.\s,]+)\s*(?:z[łl]|pln|eur|usd|€|\$)?",
        )
        .unwrap(),
        Regex::new(r"(?i)netto-betrag\s*[:\-]?\s*([\d.\s,]+)\s*(?:eur|€)?").unwrap(),
        Regex::new(r"(?i)net\s*amount\s*[:\-]?\s*([\d.\s,]+)").unwrap(),
    ]
});

static VAT_PATTERNS: LazyLock<Vec<Regex>> = LazyLock::new(|| {
    vec![
        ...
        Regex::new(r"(?i)vat\s*[:\-]\s*([\d.\s,]+)\s*(?:z[łl]|pln|eur|usd|€|\$)?").unwrap(),
        // German: "zzgl. 19% USt: 23,00 EUR" / "USt: 23,00"
        Regex::new(r"(?i)(?:zzgl\.?\s*)?\d+%?\s*ust\s*[:\-]?\s*([\d.\s,]+)\s*(?:eur|€)?").unwrap(),
    ]
});

static VAT_RATE_PATTERNS: LazyLock<Vec<Regex>> = LazyLock::new(|| {
    vec![
        // "VAT 23%" / "VAT: 23%" / "stawka VAT: 23%"
        Regex::new(r"(?i)vat\s*[:\-]?\s*(\d+(?:[.,]\d+)?)\s*%").unwrap(),
        // "19% USt" / "ust 19%"
        Regex::new(r"(?i)(\d+(?:[.,]\d+)?)\s*%\s*ust").unwrap(),
        // "Steuersatz: 19%"
        Regex::new(r"(?i)steuersatz\s*[:\-]?\s*(\d+(?:[.,]\d+)?)\s*%").unwrap(),
    ]
});
...
static DATE_PATTERNS: LazyLock<Vec<Regex>> = LazyLock::new(|| {
    vec![
        ...
        Regex::new(r"(?i)data\s+(?:wystawienia|sprzedaży|faktury)\s*[:\-]?\s*(\d{4}-\d{2}-\d{2}|\d{1,2}[.\-/]\d{1,2}[.\-/]\d{4})").unwrap(),
        // Generic: "Data: 2024-01-15"
        Regex::new(r"(?i)data\s*[:\-]\s*(\d{4}-\d{2}-\d{2}|\d{1,2}[.\-/]\d{1,2}[.\-/]\d{4})").unwrap(),
        // German: "Datum: 15.01.2024" / "Rechnungsdatum: 2024-01-15"
        Regex::new(r"(?i)(?:rechnungs)?datum\s*[:\-]?\s*(\d{1,2}[.\-/]\d{1,2}[.\-/]\d{4}|\d{4}-\d{2}-\d{2})").unwrap(),
        // English: "Date: 2024-01-15"
        Regex::new(r"(?i)date\s*[:\-]\s*(\d{4}-\d{2}-\d{2}|\d{1,2}[.\-/]\d{1,2}[.\-/]\d{4})").unwrap(),
    ]
});
...
static VENDOR_PATTERNS: LazyLock<Vec<Regex>> = LazyLock::new(|| {
    vec![
        ...
        Regex::new(r"(?i)sprzedawca\s*[:\-]\s*(.+)").unwrap(),
        // German: "Verkäufer: Thomann GmbH" / "Rechnung von: Thomann"
        Regex::new(r"(?i)(?:verkäufer|rechnung\s+von)\s*[:\-]\s*(.+)").unwrap(),
        // English: "Seller: Thomann GmbH" / "Vendor: Thomann"
        Regex::new(r"(?i)(?:seller|vendor|from)\s*[:\-]\s*(.+)").unwrap(),
    ]
});
...
fn extract_gross(text: &str) -> Option<Decimal> {
    for pattern in GROSS_PATTERNS.iter() {
...
fn extract_net(text: &str) -> Option<Decimal> {
    for pattern in NET_PATTERNS.iter() {
        if let Some(caps) = pattern.captures(text)
            && let Some(amount) = parse_amount(&caps[1])
            && amount > Decimal::ZERO
        {
...
fn extract_vat_rate(text: &str) -> Option<Decimal> {
    for pattern in VAT_RATE_PATTERNS.iter() {
        if let Some(caps) = pattern.captures(text) {
            let raw = &caps[1];
